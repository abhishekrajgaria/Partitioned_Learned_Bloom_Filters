"""
PLBF.py: It takes the learned model output for both key
and non-key values and provides the threshold value
and fpr values to partition the data in different Bloom Filters
and their false positive rate
"""

__author__ = "Kapil Vaidya, Eric Knorr, Tim Kraska, Michael Mitzenmacher"
__resource__ = "https://github.com/kapilvaidya24/PLBF/tree/main?tab=readme-ov-file"
...
def get_thresholds(key_scores, non_key_scores, num_partitions):
    ...
    num_chunks = min(10, int(len(complete_list) / 5))
    ...
    G_arr = [0.001] * num_chunks
    H_arr = [0.001] * num_chunks
    G_cummulative = [0.001] * num_chunks
    H_cummulative = [0.001] * num_chunks
    ...
    for i in range(0, num_chunks):
        # print("G_cummulative_i", G_cummulative[i])
        DP_matrix[0][i] = G_cummulative[i] * math.log(
            G_cummulative[i] * 1.00 / H_cummulative[i]
        )
        jump_matrix[0][i] = 0 - 1
    ...
    for i in range(1, num_partitions):
        for j in range(0, num_chunks):
            ...
            for k in range(0, j):
                ...
                ans = DP_matrix[i - 1][k] + (delta_G) * math.log(
                    (delta_G) * 1.00 / (delta_H)
                )
                poss_list[k] = ans
...
def get_optimal_fpr(key_scores, non_key_scores, threshold_params, target_fpr):
    ...
    G_arr = [0.001] * num_partitions
    H_arr = [0.001] * num_partitions
    ...
    while is_fpr_larger_than_one:
        ...
        for i in range(0, num_partitions):
            ...
            new_fpr = (
                ((target_fpr - H_sum) * G_arr[i]) * 1.00 / ((1 - G_sum) * H_arr[i])
            )
...
def get_parameter_vals(key_scores, non_key_scores, target_fpr, num_partitions):
    ...
    fpr_params = get_optimal_fpr(
        key_scores, non_key_scores, threshold_params, target_fpr
    )
